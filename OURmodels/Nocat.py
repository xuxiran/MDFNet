...
class Nocat(nn.Module):
    ...
    def train(self, train_loader, device, epoch, N_epoch):
        ...
        for iter, (X,label,identities) in enumerate(tqdm.tqdm(train_loader, position=0, leave=True), start=1):
            ...
            output = self.model(X[6].to(device), X[7].to(device), X[8].to(device), X[9].to(device), X[10].to(device))
            ...
            loss.backward()
            self.optimizer.step()
        print(f"Nocat Train Accuracy: {all_correct/count}")
...
class DenseBlock(nn.Module):
    ...
    def __init__(self, num_convs, in_channels, out_channels):
        ...
        self.out_channels = out_channels # get the out channels
