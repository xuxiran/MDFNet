...
# no 5 band
class Nofre(nn.Module):
    def __init__(self, device, decision_window):
        super(Nofre, self).__init__()
    ...
    def train(self, train_loader, device, epoch, N_epoch):
        ...
        for iter, (X,label,identities) in enumerate(tqdm.tqdm(train_loader, position=0, leave=True), start=1):
            ...
            output = self.model(X[2].to(device))
            ...
            loss.backward()
            self.optimizer.step()
        print(f"Nofre Train Accuracy: {all_correct/count}")
...
if __name__ == '__main__':
    ...
    x = torch.rand((10, decision_window,10,11)).to(device)
    out = model(x)
