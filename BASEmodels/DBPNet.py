...
class DBPNet(nn.Module):
    def __init__(self, device, decision_window):
        ...
        self.model = dbpnet(decision_window)
    ...
    def train(self, train_loader, device, epoch, N_epoch):
        ...
        for iter, (X, y, z) in enumerate(tqdm.tqdm(train_loader, position=0, leave=True), start=1):
            ...
            convScore = self.model(X[4].to(device), X[3][:,5:10,:,:].to(device))
            y = y.to(device)
            ...
            loss.backward()
            self.optimizer.step()
        print(f"DBPNet Train Accuracy: {all_correct/count}")
...
if __name__ == '__main__':
    ...
    decision_window = 64
    model = dbpnet(decision_window).to(device)
    x1 = torch.rand((1,  64, decision_window)).to(device)
    x2 = torch.rand((1,5, 10,10)).to(device)
